<template>
  <div class="p-8 max-w-5xl mx-auto w-full">
    ...
    <div class="flex items-center justify-between mb-8 pb-4 border-b border-gray-100">
      ...
      <button @click="load" :disabled="loading" class="flex items-center gap-2 bg-white border border-gray-200 text-[#2E2E2E] px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors shadow-sm">
    ...
    <!-- Empty -->
    <div v-else-if="pendingUsers.length === 0" class="bg-white rounded-xl border border-gray-100 shadow-sm py-16 flex flex-col items-center text-center">
      ...
      <Transition name="fade">
        <div v-if="selected" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" @click.self="selected = null">
          <Transition name="scale">
            <div v-if="selected" class="bg-white rounded-2xl shadow-2xl border border-gray-100 w-full max-w-md overflow-hidden">
              <!-- Modal Header -->
              ...
              <div class="px-6 py-5 space-y-4">
                <div class="grid grid-cols-2 gap-3">
                  ...
                  <div class="bg-amber-50 rounded-xl p-4 col-span-2 flex items-center gap-3">
                    ...
                    <p class="text-sm text-amber-700 font-medium">Awaiting admin verification</p>
                  </div>
                </div>
              </div>
